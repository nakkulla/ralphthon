...
## Scene 1 — 등록 요청 + 자동 수집 (등록 플로우 A, 0단계)

팀원은 프로파일 파일을 손으로 쓰지 않는다. 그냥 Codex에서 스킬을 부른다:

> "`profile-gallery`로 내 프로파일 등록해줘"

스킬이 **자동으로 프로젝트 정보를 수집**한다:

- **현재 레포**: `README.md`, `pyproject.toml`/`package.json`, 디렉터리 구조,
  `git remote get-url origin`, 최근 커밋, 언어 구성.
- **Codex 메모리/컨텍스트**: `AGENTS.md`, `~/.codex` 메모리, 현재 세션 지식.

(사용자가 파일/텍스트를 직접 주면 그것을 우선·보강에 쓰고, 없으면 자동 수집만으로 진행)

---

## Scene 2 — 등록 초안 + 사용자 컨펌

스킬이 수집 내용을 분석해 **등록 초안**을 만들어 사용자에게 보여주고 확인을 받는다:

```
스킬: 아래 내용으로 등록할게요. 수정할 부분 있나요?
{
  "name": "MoodBoard",
  "summary": "음악 감정 분석 기반 플레이리스트 자동 큐레이션 웹앱",
  "raw_text": "(레포 README + AGENTS.md에서 자동 조립)",
  "tech_stack": ["Next.js", "FastAPI", "pgvector", "Spotify API"],
  "domain": "music-ai",
  "tags": ["music", "ai", "web"],
  "keywords": ["감정분석", "플레이리스트", "큐레이션", "무드벡터"]
}
팀원: domain을 'music-ai'로 두고, 태그에 'recsys' 추가해줘.
스킬: 반영했습니다. 이대로 진행할까요? → (확인)
```

사용자가 **확인**해야만 다음 단계(이미지 생성·저장)로 넘어간다. 수정 요청 시 초안을
갱신하고 다시 확인받는다.
...
```
$ pgal profile create --data - --json <<'JSON'
...
```
$ pgal search --q "음악 ai" --tags music --json
...
`pgal profile get 7c3f... --json`는 `profile:{id}` 캐시를 활용한다.
...
```
$ pgal search --tech FastAPI,pgvector --tech-match all --json
NAME       SUMMARY                                  TECH_STACK                       IMAGE
MoodBoard  음악 감정 분석 기반 플레이리스트 큐레이션  next.js,fastapi,pgvector,spotify  /images/a14d...
```

백엔드는 입력을 lower/trim canonical 값으로 바꾼 뒤 `tech_stack` 배열에
`@>{fastapi,pgvector}`(all) 또는 `&&`(any) 연산으로 정확히 필터한다. FTS 부분일치가
아니라 배열 매칭이라 대소문자 표기 흔들림 없이 동작한다.
...
```
$ pgal profile update 7c3f... --data - --json <<'JSON'
...
```
"등록해줘" ─▶ 스킬 자동 수집(레포+Codex 메모리) ─▶ 등록 초안 ─▶ 사용자 컨펌
      ─▶ image_gen(이미지) ─▶ 키워드 보강
      ─▶ pgal create/image add ─▶ 백엔드 저장(PG+파일) ─▶ Valkey 캐시
      ─▶ pgal search --json(태그+FTS) ─▶ 결과+이미지 URL
      ─▶ pgal update/delete(스킬 경유) ─▶ 재색인/캐시 무효화
```

입력(등록)은 스킬이 레포·Codex 메모리에서 **자동 수집해 초안을 만들고 사용자 컨펌** 후
진행하며, 수정도 **단일 `profile-gallery` 스킬**이 `pgal` CLI를 호출해 수행한다.
백엔드는 저장·검색·서빙만 담당한다.
